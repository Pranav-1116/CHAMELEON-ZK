...
contract StateCommitmentVerifier {
    ...
    event CommitmentSubmitted(address indexed user, uint256 indexed commitment, uint8 backendId, uint256 index);

    event CommitmentVerified(address indexed user, uint256 indexed commitment, bool valid);

    constructor(address _verifier) {
        verifier = BN254Verifier(_verifier);
    }

    function submitCommitment(uint256 commitment, uint8 backendId) external returns (uint256) {
        require(!commitmentExists[commitment], "Commitment exists");
        require(backendId <= 1, "Invalid backend");

        Commitment memory newCommitment =
            Commitment({commitment: commitment, backendId: backendId, timestamp: block.timestamp, verified: false});
    ...
    function getCommitment(address user, uint256 index)
        external
        view
        returns (uint256 commitment, uint8 backendId, uint256 timestamp, bool verified)
    {
        require(index < commitmentHistory[user].length, "Invalid index");
        Commitment memory commit = commitmentHistory[user][index];
        return (commit.commitment, commit.backendId, commit.timestamp, commit.verified);
    }
