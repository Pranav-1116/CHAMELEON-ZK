...
contract UniversalVerifier {
    ...
    // Events
    event BackendMorphed(uint8 indexed fromBackend, uint8 indexed toBackend, uint256 timestamp);

    event ProofVerified(uint8 indexed proofType, uint8 indexed backend, bool success);
     ...
     */
    function setStateCommitmentVerifier(uint8 backend, address verifier) external onlyOwner {
     ...
     * @param verifier The verifier contract address
     */
    function setMorphValidatorVerifier(uint8 backend, address verifier) external onlyOwner {
        require(backend <= BACKEND_BLS12_381, "Invalid backend");
     ...
     */
    function getMorphDetails(uint256 index)
        external
        view
        returns (uint8 fromBackend, uint8 toBackend, uint256 timestamp, bytes32 stateCommitment)
    {
